	...
	<body onload="loginCheck()">
		<script>	
			...
			function goLogin() {
				...
				fetch("http://localhost1:8080/loginREST", {
					  method: "POST",
					  body: payload,
				})
			...
			function getList() {
				fetch("http://localhost:8080/listREST")
					.then((response) => response.json())
					.then((data) => {
			...
			function goLogout() {
				fetch("http://localhost:8080/logoutREST")
				.then((response) => {
					loginok = "";
					member_grade = "";
					document.getElementById("loginbox").style.display = "block";
			...
			function goDel(id) {
				if(confirm("정말 삭제하시겠습니까?")){
					fetch("http://localhost:8080/delREST?id=" + id)
					.then((response) => response.json())
					.then((data) => {
						if(data.result == "fail") {
							alert("비밀번호가 틀렸습니다.");	
			...
			function goMod(id, modPass, modName) {
				...
				fetch("http://localhost:8080/modREST?id=" + id + "&contents=" + modifyText
			...
			function goAdd() {
				...
				fetch("http://localhost:8080/addREST", {
					  method: "POST",
					  body: payload,
				})
				.then((response) => {
